...
# optimized for 56 cpu VM
# Change the number of cpus and parallel calls to suit the available resources
#
...
export parallel_run_file="run_DV.cmd"
export wd=$(pwd)
for file in *.bam;
do
    ...
    echo "# $sample" >> $parallel_run_file
    echo "docker run -v $docker_mnt google/deepvariant /opt/deepvariant/bin/run_deepvariant --model_type=WGS --ref=$genome_ref --reads=$reads --sample_name=$sample --output_vcf=$vcf --output_gvcf=$gvcf --intermediate_results_dir=$intermediate --regions '$regions' --num_shards=$cpus --logging_dir=$log" >> $parallel_run_file
    echo "" >> $parallel_run_file
done


parallel2_95.py --job-name "deepvaraint" -n $n_parallel_calls -f $parallel_run_file > $logfile 2>&1
